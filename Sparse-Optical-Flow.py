...
def normalize(hist) :
	...
	#print(valmax)
	#print(valmin)
	hist = (hist - valmin) / (valmax - valmin)
	#rows, cols = np.nonzero(hist)
	#print(hist[rows,cols])
	return hist

def histogram2d_Vx_Vy(good_old, good_new):
	flow = np.zeros_like(good_old)
	for i in range(len(good_old)) :
		flow[i] = good_new[i] - good_old[i]
...
while(ret):
	...
	hist = histogram2d_Vx_Vy(good_old, good_new)
